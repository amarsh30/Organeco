...
class CekSubsidiActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        spinnerKomoditas.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(
                ...
                Toast.makeText(
                    this@CekSubsidiActivity,
                    getString(R.string.selected_item) + " " + komoditasDisplay[position] + "nilai rill komoditas adalah" + komoditasDisplay,
                    Toast.LENGTH_LONG
                ).show()
            }
    ...
    private fun calculateResult(luas_lahan: Int, kelompok_tani: Boolean, kartu_tani: Boolean) {
        if (luas_lahan < 20000 && kelompok_tani && kartu_tani) {
            Toast.makeText(
                this@CekSubsidiActivity,
                "Anda layak mendapatkan subsidi pupuk, Anda bisa mendapatkannya pada kios pupuk resmi terdekat",
                Toast.LENGTH_LONG
            ).show()
        } else if (luas_lahan < 2000 && !kelompok_tani && kartu_tani) {
            Toast.makeText(
                this@CekSubsidiActivity,
                "Anda layak mendapatkan subsidi pupuk, Anda bisa mendapatkannya pada kios pupuk resmi terdekat dengan membawa KTP",
                Toast.LENGTH_LONG
            ).show()
        } else if (!kartu_tani) {
            Toast.makeText(
                this@CekSubsidiActivity,
                "Maaf anda harus memiliki kartu tani untuk mendapatkan subsidi, Silahkan daftar terlebih dahulu di website simulthan",
                Toast.LENGTH_LONG
            ).show()
        } else {
            Toast.makeText(
                this@CekSubsidiActivity,
                "Maaf anda belum bisa mendapatkan subsidi", Toast.LENGTH_LONG
            ).show()
        }
